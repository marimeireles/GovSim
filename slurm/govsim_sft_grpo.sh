#!/bin/bash
#SBATCH --job-name=govsim-sft-grpo
#SBATCH --account=def-zhijing
#SBATCH --time=48:00:00
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --output=slurm/logs/govsim_sft_grpo_%j.out
#SBATCH --error=slurm/logs/govsim_sft_grpo_%j.err
#SBATCH --exclude=ng31004

# ============================================================
# Two-Stage Training: SFT → GRPO
#
# Stage 1: SFT on ideal traces (teaches output distribution)
# Stage 2: GRPO on SFT checkpoint (refines with reward signal)
#
# Usage:
#   sbatch slurm/govsim_sft_grpo.sh
# ============================================================
...
SFT_OUTPUT="${SFT_OUTPUT:-models/gemma3-4b-sft-v1}"
GRPO_OUTPUT="${GRPO_OUTPUT:-models/gemma3-4b-sft-grpo-v1}"
...
echo "============================================================"
echo "Job:       GovSim SFT → GRPO Pipeline"
echo "Date:      $(date)"
echo "Node:      $(hostname)"
